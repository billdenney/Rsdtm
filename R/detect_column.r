...
###################################################################
###################################################################
# Need help on how to check if the date columns is consistent with ISO8601 format
...
validate_iso <- function(data,domain_spec){
  ...
  if(any((data[[country$'Variable Name']] %in% d.countries$a3)==F)){
    stop(
      "The following column does not follow ISO 3166-1 Alpha-3 standards: COUNTRY"
    )
  }
  }
}
